...
// Usage examples:
//
//	direct-connector --mode ssh --host jump.example.com --port 443 \
...
func runCLI() {
	...
	port := fs.String("port", envOr("DC_PORT", "443"), "SSH port (443 recommended — bypasses most firewalls)")
	...
	fwdMode := fs.String("forward-mode", envOr("DC_FWD_MODE", "R"), "Forward direction: R = remote/reverse (-R, default), L = local (-L)")
	verboseSSH := fs.Bool("verbose", false, "Enable verbose SSH logging (-v); very noisy, use only for debugging")
	useWSL := fs.Bool("wsl", false, "Windows: run tunnel via 'wsl.exe ssh' so it runs inside WSL network (where Docker/MQTT lives)")
	...
	switch *mode {
	case "ssh":
		cliSSH(logf, *host, *port, *user, *ports, *ip, *keyFlag, *fwdMode, *verboseSSH, *useWSL)
	case "p2p-consumer":
...
func cliSSH(logf func(string, ...any), host, port, user, ports, ipVer, keyPath, fwdMode string, verbose, useWSL bool) {
	if host == "" || user == "" || ports == "" {
	...
	if _, err := tunnel.BuildSSHArgs(cfg); err != nil {
	...
	tm := &tunnel.Manager{
		OnStatus: func(s tunnel.Status) { logf("Status: %s", s.String()) },
...
func cliKeygen(logf func(string, ...any)) {
	logf("Generating Ed25519 keypair in: %s", keygen.AppKeyDir())
	...
	logf("Private key: %s", keygen.AppPrivateKeyPath())
	logf("Public key:  %s", keygen.AppPublicKeyPath())
	fmt.Println("")
