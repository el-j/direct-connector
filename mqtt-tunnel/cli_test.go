...
func TestCLIKeygen_GeneratesKey(t *testing.T) {
	...
	privPath := filepath.Join(dir, appName, "id_ed25519")
	pubPath := filepath.Join(dir, appName, "id_ed25519.pub")
	...
	for _, p := range []string{privPath, pubPath} {
		if _, err := os.Stat(p); os.IsNotExist(err) {
			t.Logf("key not at expected path %s (OS may use a different config dir) — checking appPrivateKeyPath()", p)
		}
	}
